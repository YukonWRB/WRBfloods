...
if (!is.null(database)){ #Get the data from the database. Checks on database existence and type have already been performed in the .R file.
  ...
  for (i in locations){
    bridge_daily[[i]] <- DBI::dbGetQuery(hydro, paste0("SELECT value, date, max, min, QP90, QP75, QP50, QP25, QP10 FROM distance_daily WHERE location = '", i, "' AND date BETWEEN '", Sys.Date()-365, "' AND '", Sys.Date(), "'" ))
    names[i] <- stringr::str_to_title(unique(DBI::dbGetQuery(hydro, paste0("SELECT name FROM locations WHERE location = '", i, "' AND data_type = 'distance'"))))
    bridge_rt[[i]] <-  DBI::dbGetQuery(hydro, paste0("SELECT value, datetime_UTC FROM distance_realtime WHERE location = '", i, "' AND datetime_UTC BETWEEN '", .POSIXct(Sys.time(), "UTC")-zoom_days * 60*60*24, "' AND '", .POSIXct(Sys.time(), "UTC"), "'"))
  }
