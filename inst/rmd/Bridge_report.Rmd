...
if (!is.null(database)){ #Get the data from the database. Checks on database existence and type have already been performed in the .R file.
  hydro <- DBI::dbConnect(RSQLite::SQLite(), database)
  on.exit(DBI::dbDisconnect(hydro))
  DBI::dbExecute(hydro, "PRAGMA busy_timeout=60000")
  for (i in locations){
    bridge_daily[[i]] <- DBI::dbGetQuery(hydro, paste0("SELECT value, date, max, min, QP90, QP75, QP50, QP25, QP10 FROM bridge_distance_daily WHERE location = '", i, "' AND date BETWEEN '", Sys.Date()-365, "' AND '", Sys.Date(), "'" ))
    names[i] <- unique(DBI::dbGetQuery(hydro, paste0("SELECT name FROM locations WHERE location = '", i, "'")))
    bridge_rt[[i]] <-  DBI::dbGetQuery(hydro, paste0("SELECT value, datetime_UTC FROM bridge_distance_realtime WHERE location = '", i, "' AND datetime_UTC BETWEEN '", .POSIXct(Sys.time(), "UTC")-zoom_days * 60*60*24, "' AND '", .POSIXct(Sys.time(), "UTC"), "'"))
  }
...
cat("  \n# Summary of freeboard and freeboard changes \n")

# levels <- data.frame("Location" = unlist(unname(names)), "Current distance" = NA, "24 hr change (cm)" = NA, "48 hr change (cm)" = NA, "72 hr change (cm)" = NA, "1 week change (cm)" = NA, "Last data age" = NA)
...
for (i in 1:length(locations)){
  ...
  levels[i,5] <- threeday
  levels[i,6] <- week
  levels[i,7] <- paste0(round(age[1], 1), " ", units(age))
}
