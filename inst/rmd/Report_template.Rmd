...
```{r Get Precip data, echo=FALSE, results='asis', message=FALSE, warning=FALSE}

#.shp needs to be downloaded for stations in region 10, 9, or 8 depending on which stations are in the report requested (even better if generalized to other regions). last two digits before extension are the region.

#Use 6 hour products: First one is the 0100cutoff, subsequent should use the 0700cutoff. Time part of datetime object will be 00, 06, 12, 18.

#Get the links from the web, turn it into a usable table
htmlcode <- xml2::read_html("https://dd.weather.gc.ca/analysis/precip/hrdpa_watershed/shapefile/06/")
...
htmlcode <- xml2::read_html("https://dd.weather.gc.ca/analysis/precip/hrdpa_watershed/shapefile/24/")
nodes <- rvest::html_elements(htmlcode,xpath='//*[contains(@href, ".dbf")]') %>%
  rvest::html_attr("href")
...
regions <- list() #make empty list to fill iteratively for each watershed
first6 <- list()
second6 <- list()
third6 <- list()
fourth6 <- list()
...
for (i in watersheds){
  ...
  validdatetime <- as.Date(first$datetime[1], format="%Y%m%d%H") #the date/time up to which precip is integrated
  ...
  temp <- tempfile(fileext=".dbf")
  R.utils::downloadFile(paste0("https://dd.weather.gc.ca/analysis/precip/hrdpa_watershed/shapefile/06/CMC_HRDPA_WATERSHED-006-0700cutoff_SFC_0_ps2.5km_", subsequent6$datetime[1], "_000_", i, ".dbf"), temp)
  second6[[i]] <- foreign::read.dbf(temp)
  unlink(temp)
  
  temp <- tempfile(fileext=".dbf")
  R.utils::downloadFile(paste0("https://dd.weather.gc.ca/analysis/precip/hrdpa_watershed/shapefile/06/CMC_HRDPA_WATERSHED-006-0700cutoff_SFC_0_ps2.5km_", subsequent6$datetime[2], "_000_", i, ".dbf"), temp)
  third6[[i]] <- foreign::read.dbf(temp)
  unlink(temp)  
  
  temp <- tempfile(fileext=".dbf")
  R.utils::downloadFile(paste0("https://dd.weather.gc.ca/analysis/precip/hrdpa_watershed/shapefile/06/CMC_HRDPA_WATERSHED-006-0700cutoff_SFC_0_ps2.5km_", subsequent6$datetime[3], "_000_", i, ".dbf"), temp)
  fourth6[[i]] <- foreign::read.dbf(temp)
  unlink(temp)  
}

first6 <- subset(data.table::rbindlist(first6), Station %in% stations)
second6 <- subset(data.table::rbindlist(second6), Station %in% stations)
third6 <- subset(data.table::rbindlist(third6), Station %in% stations)
fourth6 <- subset(data.table::rbindlist(fourth6), Station %in% stations)

first24 <- list(first6=first6, second6=second6, third6=third6, fourth6=fourth6)


#THEN use 24 hour HRDPA to get the rest of the week.

for (i in watersheds){
  
  #this here needs to be set according to the datetime of the 24 hour periods.
  subsequent24 <- dplyr::filter(dbf24, watershed==i, cutoff=="0700") %>% dplyr::arrange(desc(datetime))
  
  temp <- tempfile(fileext=".dbf")
  R.utils::downloadFile(paste0("https://dd.weather.gc.ca/analysis/precip/hrdpa_watershed/shapefile/24/CMC_HRDPA_WATERSHED-024-0700cutoff_SFC_0_ps2.5km_", substr(subsequent24$datetime[1], 1,8), "12_000_", i, ".dbf"), temp)
  first24[[i]] <- foreign::read.dbf(temp)
  unlink(temp)
  
  temp <- tempfile(fileext=".dbf")
  R.utils::downloadFile(paste0("https://dd.weather.gc.ca/analysis/precip/hrdpa_watershed/shapefile/24/CMC_HRDPA_WATERSHED-024-0700cutoff_SFC_0_ps2.5km_", substr(subsequent24$datetime[2], 1,8), "12_000_", i, ".dbf"), temp)
  second24[[i]] <- foreign::read.dbf(temp)
  unlink(temp)
  
  temp <- tempfile(fileext=".dbf")
  R.utils::downloadFile(paste0("https://dd.weather.gc.ca/analysis/precip/hrdpa_watershed/shapefile/24/CMC_HRDPA_WATERSHED-024-0700cutoff_SFC_0_ps2.5km_", substr(subsequent24$datetime[3], 1,8), "12_000_", i, ".dbf"), temp)
  third24[[i]] <- foreign::read.dbf(temp)
  unlink(temp)  
  
  temp <- tempfile(fileext=".dbf")
  R.utils::downloadFile(paste0("https://dd.weather.gc.ca/analysis/precip/hrdpa_watershed/shapefile/24/CMC_HRDPA_WATERSHED-024-0700cutoff_SFC_0_ps2.5km_", substr(subsequent24$datetime[4], 1,8), "12_000_", i, ".dbf"), temp)
  fourth24[[i]] <- foreign::read.dbf(temp)
  unlink(temp)
  
    temp <- tempfile(fileext=".dbf")
  R.utils::downloadFile(paste0("https://dd.weather.gc.ca/analysis/precip/hrdpa_watershed/shapefile/24/CMC_HRDPA_WATERSHED-024-0700cutoff_SFC_0_ps2.5km_", substr(subsequent24$datetime[5], 1,8), "12_000_", i, ".dbf"), temp)
  fifth24[[i]] <- foreign::read.dbf(temp)
  unlink(temp)
  
    temp <- tempfile(fileext=".dbf")
  R.utils::downloadFile(paste0("https://dd.weather.gc.ca/analysis/precip/hrdpa_watershed/shapefile/24/CMC_HRDPA_WATERSHED-024-0700cutoff_SFC_0_ps2.5km_", substr(subsequent24$datetime[6], 1,8), "12_000_", i, ".dbf"), temp)
  sixth24[[i]] <- foreign::read.dbf(temp)
  unlink(temp)
  
    temp <- tempfile(fileext=".dbf")
  R.utils::downloadFile(paste0("https://dd.weather.gc.ca/analysis/precip/hrdpa_watershed/shapefile/24/CMC_HRDPA_WATERSHED-024-0700cutoff_SFC_0_ps2.5km_", substr(subsequent24$datetime[7], 1,8), "12_000_", i, ".dbf"), temp)
  seventh24[[i]] <- foreign::read.dbf(temp)
  unlink(temp)
}

first24 <- subset(data.table::rbindlist(first24), Station %in% stations)
second24 <- subset(data.table::rbindlist(second24), Station %in% stations)
third24 <- subset(data.table::rbindlist(third24), Station %in% stations)
fourth24 <- subset(data.table::rbindlist(fourth24), Station %in% stations)
fifth24 <- subset(data.table::rbindlist(fifth24), Station %in% stations)
sixtt24 <- subset(data.table::rbindlist(sixth24), Station %in% stations)
seventh24 <- subset(data.table::rbindlist(seventh24), Station %in% stations)

sevendays <- list(first24=first24, second24=second24, third24=third24, fourth24=fourth24, fitfth25=fifth24, sixth24=sixth24, seventh24=seventh24)
...
# Accumulated precipitation above selected stations

```{r Precip table, echo=FALSE, results='asis', message=FALSE}

precip12hr <- first24$first6$PR_mm + first24$second6$PR_mm
precip24hr <- precip12hr + first24$third6$PR_mm + first24$fourth6$PR_mm
precip48hr
precip72hr
precip96hr
precip1wk

precipTable <- data.frame(first24$first6$Station, stringr::str_to_title(first24$first6$StationNam), precip12hr, precip24hr, precip48hr, precip72hr, precip96hr, precip1wk)
colnames(precipTable) <- c("Station code", "Station name", "12 hours", "24 hours", "48 hours", "72 hours", "96 hours", "One week")
  
  knitr::kable(precipTable, digits=3, caption=paste0("Accumulated precipitation (mm of water) prior to ", validdatetime, " UTC"))
```
...
if (nrow(images)>=1){
  ...
  for (i in unique(images$Station)){
    ...
    cat("  \n### ", stringr::str_to_title(hy_stations(i)$STATION_NAME), "(", trimws(last_image[,2]), ")  \n")
