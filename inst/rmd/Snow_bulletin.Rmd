...
snowbullSWE <- function(loc, basin, year) {
  #swe_basin <- swe_basins %>% dplyr::filter(basin==basin)
  swe_basin <- swe_basins[swe_basins$location==basin,]
  # run hydrometDiscrete
  WRBplots::hydrometDiscrete(location = loc, parameter = "SWE", startDay = 1, tzone = "MST", years = c(year), title = FALSE, plot_type = "boxplot", plot_scale = 1, save_path = NULL, discrete_data = swe_basin)
...
swe_basins <- WRBfloods::SWE_basin(year <- 2022,
                                   month <- c(3, 4, 5), # c(3)
                                   threshold <- 6,
                                   csv = FALSE,
                                   summarise = FALSE)
