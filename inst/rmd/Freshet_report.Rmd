...
```{r Calculate station info, echo=FALSE, results='asis', message=FALSE}
...
stationInfo <- stationInfo[c(7,3,4,5)]

stationInfo <- plyr::rename(stationInfo, c("LATITUDE"="Latitude", "LONGITUDE"="Longitude", "DATUM_TO"="Datum"))

#shrink the datum names
sub <- gsub("APPROXIMATE GEODETIC SURVEY OF CANADA DATUM", "CGVD28 (approximate) [2]", stationInfo$Datum)
sub <- gsub("GEODETIC SURVEY OF CANADA DATUM", "CGVD28 (assumed) [1]", sub)
sub <- gsub("CANADIAN GEODETIC VERTICAL DATUM 2013:EPOCH2010", "CGVD2013:2010 [3]", sub)
stationInfo$Datum <- gsub("Local datum", "Local datum [4]", sub)
```
...
if (report_type %in% c("Both", "Level", "both", "level") & length(levelInfo) > 0){
  ...
  levelTable <-  dplyr::bind_cols(stationInfo[,1], levelTable)
...
if (FALSE %in% stringr::str_detect(stationInfo$Datum, "CGVD2013")){
  cat("Current water levels are presented relative to the local datum (column 2) and relative to either the Canadian Geodetic Vertical Datum 2013 (CGVD2013) or to the Canadian Geodetic Vertical datum of 1928 (CGVD28) (column 3). The local datum is an arbitrary datum used at each hydrometric station and reported by the Water Survey of Canada at [wateroffice.ec.gc.ca](https://wateroffice.ec.gc.ca/). Local datum values can be compared at one station across years but not across stations, while CGVD2013/CGVD28 elevations can be compared across stations with caution. Note that significant differences (decameter or more) may exist between CGVD2013 and CGVD28! More information on datums is provided in a table of station information below.")
} else {
  cat("Current water levels are presented relative to the local datum (column 3) and relative to the Canadian Geodetic Vertical Datum 2013 (CGVD2013, column 4). The local datum is an arbitrary datum used at each hydrometric station and reported by the Water Survey of Canada at [wateroffice.ec.gc.ca](https://wateroffice.ec.gc.ca/). Local datum values can be compared at one station across years but not across stations, while CGVD2013 elevations can be compared across stations. Older (deprecated) vertical datums are published for each station on the Water Survey of Canada website.")
}

```

Return periods (or repeat intervals) representing two, ten, one hundred and two hundred year returns are reported here (column 6) and plotted in the graphs below. A given return period refers to the expected frequency at which a specific flow or level will occur based on statistical analysis of historical records. For example, a 20-year return is expected to be met or exceeded once every 20 years or to have a 5% change of being met/exceeded every year.
...
#TODO: get the notes below to appear only if the datum is specified, and to have the right [1] number
```

Notes: 

[1] Vertical datum not specified in historical records, but implementation date indicates use of CGVD28.

[2] Derived from a digital elevation model with vertical datum in Canadian Geodetic Vertical Datum of 1928 ([CGVD28](https://cgrsc.ca/resources/geodetic-control-networks/vertical-control-networks/)); uncertainty in accordance with the [Canadian Digital Elevation Model](https://ftp.maps.canada.ca/pub/nrcan_rncan/elevation/cdem_mnec/doc/CDEM_product_specs.pdf).

[3] Vertical datum in Canadian Geodetic Vertical Datum of 2013, epoch 2010 [(CGVD2013:2010](https://cgrsc.ca/resources/geodetic-control-networks/vertical-control-networks/)).

[4] Arbitrary datum relative to station or other nearby fixed point.
