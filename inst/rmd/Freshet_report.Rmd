  ...
  for(i in stations) {
    ...
    if (stringr::str_detect(station_info$STATION_NAME, "LAKE")==FALSE | stringr::str_detect(station_info$STATION_NAME, "RIVER")==TRUE){ #Lakes don't have flow measurements, but sometimes LAKE is mentioned when describing the river location
      ...
      tryCatch ({
        ...
        startYearFlow <- min(lubridate::year(flowData$tidyData[[1]]$Date), na.rm = T)
        todayFlow <- flowData$tidyData[[3]]$Value[flowData$tidyData[[3]]$Date==Sys.Date()]
        todayFlowPerc <- round(flowData$tidyData[[3]]$prctile[flowData$tidyData[[3]]$Date==Sys.Date()],1)
        yesterdayFlow <- flowData$tidyData[[3]]$Value[flowData$tidyData[[3]]$Date==(Sys.Date()-1)]
        twodayFlow <- flowData$tidyData[[3]]$Value[flowData$tidyData[[3]]$Date==(Sys.Date()-2)]
        threedayFlow <- flowData$tidyData[[3]]$Value[flowData$tidyData[[3]]$Date==(Sys.Date()-3)]
        fourdayFlow <- flowData$tidyData[[3]]$Value[flowData$tidyData[[3]]$Date==(Sys.Date()-4)]
        fivedayFlow <- flowData$tidyData[[3]]$Value[flowData$tidyData[[3]]$Date==(Sys.Date()-5)]
        sixdayFlow <- flowData$tidyData[[3]]$Value[flowData$tidyData[[3]]$Date==(Sys.Date()-6)]
        weekFlow <- flowData$tidyData[[3]]$Value[flowData$tidyData[[3]]$Date==(Sys.Date()-7)]
...
For inquiries related to water level and iceflow conditions, contact the following person: 
Holly Goulding, Senior Hydrologist, 867-667-3223 or holly.goulding@yukon.ca

For inquiries related to flood response, contact the following organization: 
Yukon Emergency Measures Organization, 867-667-5220 or EMO.yukon@yukon.ca
