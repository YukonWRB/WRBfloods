...
```{r code save, eval=FALSE, include=FALSE}
...
if (report_name %in% c("Southern Lakes", "Southern lakes", "southern lakes")){
  cat("  \nWater levels in the Southern Lakes are driven by a combination of snowmelt, summer precipitation and glacier melt. Winter 2021-22 snow conditions suggested that water levels would be higher than average this summer. Weather conditions through the summer will determine the peak water level in the Southern Lakes, which typically occurs in late summer in response to peak glacial runoff and precipitation events.  \n  \nThis report provides an overview of current conditions.")
} else {
  cat("  \nRecord-setting spring snowpack across many Yukon watersheds had created high potential for flooding near streams and some lakes driven by snowmelt this spring and early summer. Now that the bulk of last winter's snow has melted flooding potential shifts to larger lake systems, particularly those receiving glacial meltwater such as the Southern Lakes.  \n  \nThis report provides an overview of current conditions at selected stations across the territory. More detailed reports are also issued periodically for regions experiencing high or rapidly rising water levels and flows.")
}
```
  ...
  for (i in stations) {
    ...
    tryCatch ({
      ...
      if (datum_na==TRUE) { #then there is no datum elevation
        ...
        todayLevel <- mean(dplyr::filter(levelData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(levelData[[3]]$Date) - 60*60), to = as.POSIXct(max(levelData[[3]]$Date)), by = "5 min"))$Level)
        todayLevelmasl <- NA
        ...
        startYearLevel <- min(lubridate::year(levelData[[1]]$Date), na.rm = T)
        todayLevel <- mean(dplyr::filter(levelData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(levelData[[3]]$Date) - 60*60), to = as.POSIXct(max(levelData[[3]]$Date)), by = "5 min"))$Level)
        todayLevelmasl <- mean(dplyr::filter(levelData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(levelData[[3]]$Date) - 60*60), to = as.POSIXct(max(levelData[[3]]$Date)), by = "5 min"))$Level_masl)
      ...
      #next few lines calculate where we're at for level return periods and if there's a flag raised
      if (i %in% data$level_returns$ID==TRUE) {
        ...
        tbl$Flag <- if (todayLevel >= stn$Flag_Level & !is.na(stn$Flag_Level)) {
          stn$Note
        } else {
          tbl$Flag <- "NA"
        }
    ...
    #Determine if the last data point is >6 hours old
      ...
      if (levelData[[3]]$Date[1] < (Sys.time() - 60*60*2)) {
        tbl$Last_data <- "2-3 hrs"
      }
      if (levelData[[3]]$Date[1] < (Sys.time() - 60*60*3)) {
        tbl$Last_data <- "3-4 hrs"
      }
      if (levelData[[3]]$Date[1] < (Sys.time() - 60*60*4)) {
        tbl$Last_data <- "4-5 hrs"
      }
      if (levelData[[3]]$Date[1] < (Sys.time() - 60*60*5)) {
        tbl$Last_data <- "5-6 hrs"
      }
      if (levelData[[3]]$Date[1] < (Sys.time() - 60*60*6)) {
        tbl$Last_data <- "> 6 hrs"
      }
  ...
  for(i in stations) {
    ...
    if (stringr::str_detect(station_info$STATION_NAME, "LAKE")==FALSE | stringr::str_detect(station_info$STATION_NAME, "RIVER")==TRUE){ #Lakes don't have flow measurements, but sometimes LAKE is mentioned when describing the river location
      ...
      tryCatch ({
        ...
        yesterdayFlow <- flowData[[3]]$Flow[flowData[[3]]$Date == max(flowData[[3]]$Date)-60*60*24]
        twodayFlow <- flowData[[3]]$Flow[flowData[[3]]$Date == max(flowData[[3]]$Date)-60*60*48]
        threedayFlow <- flowData[[3]]$Flow[flowData[[3]]$Date == max(flowData[[3]]$Date)-60*60*72]
        fourdayFlow <- flowData[[3]]$Flow[flowData[[3]]$Date == max(flowData[[3]]$Date)-60*60*96]
        fivedayFlow <- flowData[[3]]$Flow[flowData[[3]]$Date == max(flowData[[3]]$Date)-60*60*120]
        sixdayFlow <- flowData[[3]]$Flow[flowData[[3]]$Date == max(flowData[[3]]$Date)-60*60*144]
        weekFlow <- flowData[[3]]$Flow[flowData[[3]]$Date == max(flowData[[3]]$Date)-60*60*168]
        ...
        if (!(flow_returns %in% c("None", "none"))){ #if flow_returns is "none", the column is simply not created.
          
          if (flow_returns %in% c("both", "Both", "Table", "table") & i %in% data$flow_returns$ID){ #where possible, plug in the verified value
          ...
          } else if (flow_returns %in% c("both", "Both", "Auto", "auto")){ #if auto selected, run code OR if both selected and no human-verified value available.
            ...
            tbl$Current_return_period <- 
              if (tail(freq, n=1)$`Return Period` < 2) {
                "<2 year*"
              } else if (tail(freq, n=1)$`Return Period` < 5) {
                " 2 to 5 year*"
              } else if (tail(freq, n=1)$`Return Period` < 10) {
                "5 to 10 year*"
              } else if(tail(freq, n=1)$`Return Period` < 20) {
                "10 to 20 year*"
              } else if (tail(freq, n=1)$`Return Period` < 50) {
                "20 to 50 year*"
              } else if (tail(freq, n=1)$`Return Period` < 100) {
                "50 to 100 year*"
              } else if (tail(freq, n=1)$`Return Period` < 200) {
                "100 to 200 year*"
              } else if (tail(freq, n=1)$`Return Period` < 500) {
                "200 to 500 year*"
              } else if (tail(freq, n=1)$`Return Period` < 1000) {
                "500 to 100 year*"
              } else if (tail(freq, n=1)$`Return Period` < 2000) {
                "1000 to 2000 year*"
              } else if (tail(freq, n=1)$`Return Period` > 2000) {
                "> 2000 year*"
              }
        ...
        #Determine if the last data point is >6 hours old
        tbl$Last_data <- " "
        if (flowData[[3]]$Date[1] > (Sys.time() - 60*60*1)) {
          tbl$Last_data <- "< 1 hrs"
        }
        if (flowData[[3]]$Date[1] < (Sys.time() - 60*60*1)) {
          tbl$Last_data <- "1-2 hrs"
        }
        if (flowData[[3]]$Date[1] < (Sys.time() - 60*60*2)) {
          tbl$Last_data <- "2-3 hrs"
        }
        if (flowData[[3]]$Date[1] < (Sys.time() - 60*60*3)) {
          tbl$Last_data <- "3-4 hrs"
        }
        if (flowData[[3]]$Date[1] < (Sys.time() - 60*60*4)) {
          tbl$Last_data <- "4-5 hrs"
        }
        if (flowData[[3]]$Date[1] < (Sys.time() - 60*60*5)) {
          tbl$Last_data <- "5-6 hrs"
        }
        if (flowData[[3]]$Date[1] < (Sys.time() - 60*60*6)) {
          tbl$Last_data <- "> 6 hrs"
        }
...
# Summary of water levels and trajectory
...
if (rate == TRUE & report_type %in% c("Both", "Level", "both", "level")){
  ...
  if (report_name %in% c("Southern Lakes", "Southern lakes", "southern lakes")){
    #adjust elevations to CGVD28 (all are in 2013 at this point)
    convert <- vector()
    for (i in stations){ #CGVD2013 minus CGVD28
      convert[i] <- as.numeric(tail(tidyhydat::hy_stn_datum_conv(i)[,4], n=1)) - as.numeric(head(tidyhydat::hy_stn_datum_conv(i)[,4], n=1))
    }
  ...
  rateTable <- dplyr::bind_cols(stationInfo[,1], info[, c(2:4)], Trajectory=Trajectory)
  colnames(rateTable) <- c("Station name (number)", "Current level (m)", "Current level (masl)*", "24 hr change (cm)", "Trajectory")
...
if (report_type %in% c("Both", "Flow", "both", "flow") & length(flowInfo) > 0){
  ...
  flowTable <- dplyr::filter(flowTable, Current_flow != 0.0001)[c(1, 2, 4, 5, 7, 11, 16)] #0.0001 is the indicator for lake stations, remove those
      ...
      if (report_type %in% c("Both", "Level", "both", "level")){
        tryCatch ({
          ...
          if (level_zoom == TRUE){ #Plot zoomed-in level data
            ...
            #Lines below can be used to incorporate MESH outputs to lakes, once the files are available from Mohamed.
            # if (i %in% names(MESH) & i %in% names(CLEVER)){
            #   #Add in the MESH and CLEVER data, then nudge it to match WSC at forecast time.
            #   
            #   last_value <- tail(levelDataList[[n]][[3]], n=1)
            ...
            #   diff <- mean(nrst_MESH$MESH_prediction) - last_value$Level #Use the mean of MESH prediction in case the range compasses two points
            #   MESH[[i]]$MESH_prediction <- MESH[[i]]$MESH_prediction - diff #Adjust the data
            #   
            #   #CLEVER seems to be well adjusted so far! If that changes the adjustement should take place here.
            #   
            #   zoom <- dplyr::full_join(levelDataList[[n]][[3]], MESH[[i]], by = c("Date" = "datetime", "DateOnly" = "DateOnly"))
            #   zoom <- dplyr::full_join(zoom, CLEVER[[i]], by = c("Date" = "datetime", "DateOnly" = "DateOnly"))
            #   
            #   zoomLevelPlot <- utils_fcast_level_plot(station_number = i,
            #                                          level_years = levelDataList[[n]][[2]],
            #                                          zoom_data = zoom,
            #                                          zoom_days = zoom_days)
            #   
            ...
            #   dttm_range <- seq.POSIXt(last_value$Date-60*30, last_value$Date+60*30, by="5 min") #Make a range because the points do not line up perfectly
            #   nrst_MESH <- MESH[[i]][MESH[[i]]$datetime %in% dttm_range,] #Get the model prediction closest to that time
            #   diff <- mean(nrst_MESH$MESH_prediction) - last_value$Level #Use the mean of MESH prediction in case the range encompass two points
            #   MESH[[i]]$MESH_prediction <- MESH[[i]]$MESH_prediction - diff #Adjust the data
            #   
            #   zoom <- dplyr::full_join(levelDataList[[n]][[3]], MESH[[i]], by = c("Date" = "datetime", "DateOnly" = "DateOnly"))
            #   
            ...
            #   zoom <- dplyr::full_join(levelDataList[[n]][[3]], CLEVER[[i]], by = c("Date" = "datetime", "DateOnly" = "DateOnly"))
            #   
            #   zoomLevelPlot <- utils_fcast_level_plot(station_number = i,
            #                                          level_years = levelDataList[[n]][[2]],
            #                                          zoom_data = zoom,
          ...
          tryCatch ({
            ...
            if (flow_zoom == TRUE) { #plot zoomed-in flow data as well
              ...
              if (i %in% names(MESH) & i %in% names(CLEVER)){
                ...
                last_value <- tail(flowDataList[[n]][[3]], n=1)
                dttm_range <- seq.POSIXt(last_value$Date-60*30, last_value$Date+60*30, by="5 min") #Make a range because the points do not line up perfectly
                nrst_MESH <- MESH[[i]][MESH[[i]]$datetime %in% dttm_range,] #Get the model prediction closest to that time
