...
Record-setting May 1 snowpack in many watersheds in the territory creates a high potential for spring snowmelt driven flooding in Yukon communities. Critical during the snowmelt period are snowpack conditions, weather conditions, and water levels and flows in rivers and lakes. The Government of Yukon maintains water monitoring stations at locations across the Yukon to track water levels and flow rates in areas of potential flood risk for communities. This report provides current conditions, outlook, and technical data.
...
While much of the valley bottom snow has melted across the territory, substantial snowpack still exists at elevation.

## Weather conditions

A colder than normal start to May has followed a colder than normal April. While this cold weather allowed for a slow and mostly uneventful thermal breakup on Yukon rivers, it has also preserved the high elevation snowpack. 

## Water conditions

Water levels are currently near or below average across most of the territory. 
...
A warming trend is forecast for the long weekend followed by a cooling trend to below seasonal temperatures. Precipitation is expected to move into central and southern Yukon on Sunday and bring the greatest amounts on Monday, mainly in the Klondike, Stewart and Central Yukon River basins.

## Water level outlook

Water levels are expected to continue to rise. The rate of rise will increase in response to the warm weather and precipitation into mid next week, but it expected to slow down again later in the week with cooler temperatures. The weekend weather is not expected to elevate water to flood levels on larger rivers, but smaller watercourses may experience bank full or overbank conditions.  
...
For water level and flow conditions: Water Resources Branch Forecasting Team: [waterlevels\@yukon.ca](mailto:waterlevels@yukon.ca){.email} For flood response: Yukon Emergency Measures Organization: 867-667-5220 or [EMO.yukon\@yukon.ca](mailto:EMO.yukon@yukon.ca){.email}
...
if (report_type %in% c("Both", "Level", "both", "level")){
  ...
  for(i in stations) {
    ...
    tbl <- data.frame(
      ...
      `Six days ago` = NA,
      `One week ago` = NA,
      `One week change cm` = NA,
      `Datum elevation m` = NA
    )
    ...
    tryCatch ({
      ...
      if (datum_na==TRUE) {
        ...
        todayLevel <- levelData[[2]]$Level[levelData[[2]]$Date == Sys.Date() & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        todayLevelmasl <- NA
        todayLevelPerc <- round(levelData[[2]]$prctile[levelData[[2]]$Date == Sys.Date() & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())], 1)
        yesterdayLevel <- levelData[[2]]$Level[levelData[[2]]$Date == (Sys.Date() - 1) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        twodayLevel <- levelData[[2]]$Level[levelData[[2]]$Date == (Sys.Date() - 2) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        threedayLevel <- levelData[[2]]$Level[levelData[[2]]$Date == (Sys.Date() - 2) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        fourdayLevel <- levelData[[2]]$Level[levelData[[2]]$Date == (Sys.Date() - 4) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        fivedayLevel <- levelData[[2]]$Level[levelData[[2]]$Date == (Sys.Date() - 5) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        sixdayLevel <- levelData[[2]]$Level[levelData[[2]]$Date == (Sys.Date() - 6) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        weekLevel <- levelData[[2]]$Level[levelData[[2]]$Date == (Sys.Date() - 7) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        ...
        startYearLevel <- min(lubridate::year(levelData[[1]]$Date), na.rm = T)
        todayLevel <- levelData[[2]]$`Level`[levelData[[2]]$Date == Sys.Date() & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        todayLevelmasl <- levelData[[2]]$`Level masl`[levelData[[2]]$Date == Sys.Date() & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        todayLevelPerc <- round(levelData[[2]]$prctile[levelData[[2]]$Date == Sys.Date() & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())], 1)
        yesterdayLevel <- levelData[[2]]$`Level masl`[levelData[[2]]$Date == (Sys.Date() - 1) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        twodayLevel <- levelData[[2]]$`Level masl`[levelData[[2]]$Date == (Sys.Date() - 2) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        threedayLevel <- levelData[[2]]$`Level masl`[levelData[[2]]$Date == (Sys.Date() - 2) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        fourdayLevel <- levelData[[2]]$`Level masl`[levelData[[2]]$Date == (Sys.Date() - 4) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        fivedayLevel <- levelData[[2]]$`Level masl`[levelData[[2]]$Date == (Sys.Date() - 5) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        sixdayLevel <- levelData[[2]]$`Level masl`[levelData[[2]]$Date == (Sys.Date() - 6) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        weekLevel <- levelData[[2]]$`Level masl`[levelData[[2]]$Date == (Sys.Date() - 7) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
      ...
      if (i %in% data$return_periods$ID==TRUE) {
        ...
        if (datum_na == FALSE){
          levelConvert <- as.numeric(dplyr::slice_tail(as.data.frame(tidyhydat::hy_stn_datum_conv(i)[,4])))
          stn <- dplyr::filter(data$return_periods, ID == i) %>%
             dplyr::mutate_if(is.numeric, ~.+ levelConvert) #modify the return intervals with the same datum as the database
  ...
  for(i in stations) {
    ...
    if (stringr::str_detect(station_info$STATION_NAME, "LAKE")==FALSE | stringr::str_detect(station_info$STATION_NAME, "RIVER")==TRUE){ #Lakes don't have flow measurements, but sometimes LAKE is mentioned when describing the river location
      ...
      tryCatch ({
        ...
        startYearFlow <-
          min(lubridate::year(flowData[[1]]$Date), na.rm = T)
        todayFlow <- flowData$Flow[flowData$Date == Sys.Date() & flowData$Year_Real == lubridate::year(Sys.Date())]
        todayFlowPerc <- flowData$prctile[flowData$Date == Sys.Date() & flowData$Year_Real == lubridate::year(Sys.Date())]
        yesterdayFlow <- flowData$Flow[flowData$Date == (Sys.Date() - 1) & flowData$Year_Real == lubridate::year(Sys.Date())]
        twodayFlow <- flowData$Flow[flowData$Date == (Sys.Date() - 2) & flowData$Year_Real == lubridate::year(Sys.Date())]
        threedayFlow <- flowData$Flow[flowData$Date == (Sys.Date() - 3) & flowData$Year_Real == lubridate::year(Sys.Date())]
        fourdayFlow <- flowData$Flow[flowData$Date == (Sys.Date() - 4) & flowData$Year_Real == lubridate::year(Sys.Date())]
        fivedayFlow <- flowData$Flow[flowData$Date == (Sys.Date() - 5) & flowData$Year_Real == lubridate::year(Sys.Date())]
        sixdayFlow <- flowData$Flow[flowData$Date == (Sys.Date() - 6) & flowData$Year_Real == lubridate::year(Sys.Date())]
        weekFlow <- flowData$Flow[flowData$Date == (Sys.Date() - 7) & flowData$Year_Real == lubridate::year(Sys.Date())]
...
if (report_type %in% c("Both", "Level", "both", "level") & length(levelInfo) > 0){
  ...
  levelTable <- dplyr::bind_rows(levelInfo)[c(5,6, 8, 11, 18)]
...
if (report_type %in% c("Both", "Flow", "both", "flow") & length(flowInfo) > 0){
  ...
  flowTable <- dplyr::filter(flowTable, Current.flow != 0.0001)[c(1, 5, 6, 8, 11, 16)] #remove the rows where 
...
for(i in stations) {
  ...
  if(is.na(station_info$STATION_NUMBER[1])==FALSE){
    tryCatch ( {
      ...
      if (report_type %in% c("Both", "Level", "both", "level")){
        tryCatch ({
          datum_na <- dplyr::slice_tail(as.data.frame(tidyhydat::hy_stn_datum_conv(i)[,4])) #different axis text if no datum
...
if (precip == TRUE){
  ...
  knitr::kable(precipTable, digits=1, caption=paste0("Accumulated precipitation (mm of water) prior to ", validdatetime-7*60*60, " Yukon Time for where available."))
...
Hydrometric stations are maintained by the Water Survey of Canada, and funded in partnership with the Government of Yukon. More information on datums can be found on the WaterOffice.ec.gc.ca website for each individual station linked in the table below. To learn more about hydrometric data please see [https://wateroffice.ec.gc.ca/contactus/faq_e.html](https://wateroffice.ec.gc.ca/contactus/faq_e.html).
