...
The Government of Yukon partners with the Government of Canada to maintain maintains water monitoring stations at locations across the Yukon, allowing the tracking of water water levels and flow rates in areas that inform potential flood risk for communities.
  ...
  for(i in stations) {
    ...
    tryCatch ({
      ...
      if (datum_na==TRUE) {
        ...
        yesterdayLevel <- mean(dplyr::filter(levelData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(levelData[[3]]$Date)-60*60*24 - 60*30), to = as.POSIXct(max(levelData[[3]]$Date)-60*60*24 + 60*30), by = "5 min"))$Level)
        twodayLevel <- mean(dplyr::filter(levelData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(levelData[[3]]$Date)-60*60*48 - 60*30), to = as.POSIXct(max(levelData[[3]]$Date)-60*60*48 + 60*30), by = "5 min"))$Level)
        threedayLevel <- mean(dplyr::filter(levelData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(levelData[[3]]$Date)-60*60*72 - 60*30), to = as.POSIXct(max(levelData[[3]]$Date)-60*60*72 + 60*30), by = "5 min"))$Level)
        fourdayLevel <- mean(dplyr::filter(levelData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(levelData[[3]]$Date)-60*60*96 - 60*30), to = as.POSIXct(max(levelData[[3]]$Date)-60*60*96 + 60*30), by = "5 min"))$Level)
        fivedayLevel <- mean(dplyr::filter(levelData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(levelData[[3]]$Date)-60*60*120 - 60*30), to = as.POSIXct(max(levelData[[3]]$Date)-60*60*120 + 60*30), by = "5 min"))$Level)
        sixdayLevel <- mean(dplyr::filter(levelData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(levelData[[3]]$Date)-60*60*144 - 60*30), to = as.POSIXct(max(levelData[[3]]$Date)-60*60*144 + 60*30), by = "5 min"))$Level)
        weekLevel <- mean(dplyr::filter(levelData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(levelData[[3]]$Date)-60*60*168 - 60*30), to = as.POSIXct(max(levelData[[3]]$Date)-60*60*168 + 60*30), by = "5 min"))$Level)
        ...
        yesterdayLevel <- mean(dplyr::filter(levelData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(levelData[[3]]$Date)-60*60*24 - 60*30), to = as.POSIXct(max(levelData[[3]]$Date)-60*60*24 + 60*30), by = "5 min"))$Level_masl)
        twodayLevel <- mean(dplyr::filter(levelData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(levelData[[3]]$Date)-60*60*48 - 60*30), to = as.POSIXct(max(levelData[[3]]$Date)-60*60*48 + 60*30), by = "5 min"))$Level_masl)
        threedayLevel <- mean(dplyr::filter(levelData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(levelData[[3]]$Date)-60*60*72 - 60*30), to = as.POSIXct(max(levelData[[3]]$Date)-60*60*72 + 60*30), by = "5 min"))$Level_masl)
        fourdayLevel <- mean(dplyr::filter(levelData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(levelData[[3]]$Date)-60*60*96 - 60*30), to = as.POSIXct(max(levelData[[3]]$Date)-60*60*96 + 60*30), by = "5 min"))$Level_masl)
        fivedayLevel <- mean(dplyr::filter(levelData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(levelData[[3]]$Date)-60*60*120 - 60*30), to = as.POSIXct(max(levelData[[3]]$Date)-60*60*120 + 60*30), by = "5 min"))$Level_masl)
        sixdayLevel <- mean(dplyr::filter(levelData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(levelData[[3]]$Date)-60*60*144 - 60*30), to = as.POSIXct(max(levelData[[3]]$Date)-60*60*144 + 60*30), by = "5 min"))$Level_masl)
        weekLevel <- mean(dplyr::filter(levelData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(levelData[[3]]$Date)-60*60*168 - 60*30), to = as.POSIXct(max(levelData[[3]]$Date)-60*60*168 + 60*30), by = "5 min"))$Level_masl)
  ...
  for(i in stations) {
    ...
    if (stringr::str_detect(station_info$STATION_NAME, "LAKE")==FALSE | stringr::str_detect(station_info$STATION_NAME, "RIVER")==TRUE){ #Lakes don't have flow measurements, but sometimes LAKE is mentioned when describing the river location
      ...
      tryCatch ({
        ...
        yesterdayFlow <- mean(dplyr::filter(flowData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(flowData[[3]]$Date)-60*60*24 - 60*30), to = as.POSIXct(max(flowData[[3]]$Date)-60*60*24 + 60*30), by = "5 min"))$Flow)
        twodayFlow <- mean(dplyr::filter(flowData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(flowData[[3]]$Date)-60*60*48 - 60*30), to = as.POSIXct(max(flowData[[3]]$Date)-60*60*48 + 60*30), by = "5 min"))$Flow)
        threedayFlow <- mean(dplyr::filter(flowData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(flowData[[3]]$Date)-60*60*72 - 60*30), to = as.POSIXct(max(flowData[[3]]$Date)-60*60*72 + 60*30), by = "5 min"))$Flow)
        fourdayFlow <- mean(dplyr::filter(flowData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(flowData[[3]]$Date)-60*60*96 - 60*30), to = as.POSIXct(max(flowData[[3]]$Date)-60*60*96 + 60*30), by = "5 min"))$Flow)
        fivedayFlow <- mean(dplyr::filter(flowData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(flowData[[3]]$Date)-60*60*120 - 60*30), to = as.POSIXct(max(flowData[[3]]$Date)-60*60*120 + 60*30), by = "5 min"))$Flow)
        sixdayFlow <- mean(dplyr::filter(flowData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(flowData[[3]]$Date)-60*60*144 - 60*30), to = as.POSIXct(max(flowData[[3]]$Date)-60*60*144 + 60*30), by = "5 min"))$Flow)
        weekFlow <- mean(dplyr::filter(flowData[[3]], Date %in% seq.POSIXt(from = as.POSIXct(max(flowData[[3]]$Date)-60*60*168 - 60*30), to = as.POSIXct(max(flowData[[3]]$Date)-60*60*168 + 60*30), by = "5 min"))$Flow)
...
if (report_type %in% c("Both", "Flow", "both", "flow") & length(flowInfo) > 0){
  ...
  flowTable <- dplyr::filter(flowTable, Current_flow != 0.0001)[c(1, 5, 6, 8, 12, 18)] #0.0001 is the indicator for lake stations, remove those
  ...
  flowTable <- dplyr::relocate(flowTable, Community)
    ...
    colnames(flowTable) <- c("Community", "Station name (number)", "Current flow (m^3^/s)", "Current percent of historic max", "24 hr change (m^3^/s)", "72 hr change (m^3^/s)")
  
  print(knitr::kable(flowTable, digits=1))
  
  if (NA %in% stringr::str_detect(flowTable$Current_flow, "NA")){
...
tryCatch({
  ...
  knitr::kable(precipTable, digits=1, caption=paste0("Accumulated precipitation (mm of water) prior to ", validdatetime-7*60*60, " Yukon Time where available."))}, error=function(e) {cat("  \nPrecipitation shapefiles could not be downloaded of processed from the following URL: https://dd.weather.gc.ca/analysis/precip/hrdpa_watershed/shapefile/06/. Check to see if it can be reached, if that checks out ask Ghislain to look further into the problem.")})
...
Hydrometric stations are maintained by the Water Survey of Canada and funded in partnership with the Government of Yukon. Additional station information, including on vertical datums can be found at [https://wateroffice.ec.gc.ca](https://wateroffice.ec.gc.ca). To learn more about hydrometric data in general please see [https://wateroffice.ec.gc.ca/contactus/faq_e.html](https://wateroffice.ec.gc.ca/contactus/faq_e.html).
