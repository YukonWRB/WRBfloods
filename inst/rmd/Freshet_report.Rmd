...
The Government of Yukon maintains water monitoring stations at locations across the Yukon, allowing the tracking of water water levels and flow rates in areas that inform potential flood risk for communities. 
    ...
    tryCatch ({
      ...
      if (datum_na==TRUE) {
        ...
        tbl <- data.frame(
          ...
          `24_hr_change_cm` = (todayLevel - yesterdayLevel)*100,
          Two_days_ago = twodayLevel,
          Three_days_ago = threedayLevel,
          `72_hr_change_cm` = (todayLevel - threedayLevel)*100,
          Four_days_ago = fourdayLevel,
          Five_days_ago = fivedayLevel,
          Six_days_ago = sixdayLevel,
          One_week_ago = weekLevel,
          One_week_change_cm = (todayLevel - weekLevel)*100,
        ...
        startYearLevel <- min(lubridate::year(levelData[[1]]$Date), na.rm = T)
        todayLevel <- levelData[[2]]$`Level`[levelData[[2]]$Date == Sys.Date() & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        todayLevelmasl <- levelData[[2]]$Level_masl[levelData[[2]]$Date == Sys.Date() & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        todayLevelPerc <- round(levelData[[2]]$prctile[levelData[[2]]$Date == Sys.Date() & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())], 1)
        yesterdayLevel <- levelData[[2]]$Level_masl[levelData[[2]]$Date == (Sys.Date() - 1) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        twodayLevel <- levelData[[2]]$Level_masl[levelData[[2]]$Date == (Sys.Date() - 2) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        threedayLevel <- levelData[[2]]$Level_masl[levelData[[2]]$Date == (Sys.Date() - 2) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        fourdayLevel <- levelData[[2]]$Level_masl[levelData[[2]]$Date == (Sys.Date() - 4) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        fivedayLevel <- levelData[[2]]$Level_masl[levelData[[2]]$Date == (Sys.Date() - 5) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        sixdayLevel <- levelData[[2]]$Level_masl[levelData[[2]]$Date == (Sys.Date() - 6) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        weekLevel <- levelData[[2]]$Level_masl[levelData[[2]]$Date == (Sys.Date() - 7) & levelData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        ...
        tbl <- data.frame(
          ...
          Level_masl = todayLevelmasl,
          Yesterday = yesterdayLevel,
          `24_hr_change_cm` = (todayLevelmasl - yesterdayLevel)*100,
          Two_days_ago = twodayLevel,
          Three_days_ago = threedayLevel,
          `72_hr_change_cm` = (todayLevelmasl - threedayLevel)*100,
          Four_days_ago = fourdayLevel,
          Five_days_ago = fivedayLevel,
          Six_days_ago = sixdayLevel,
          One_week_ago = weekLevel,
          One_week_change_cm = (todayLevelmasl - weekLevel)*100,
          Datum_elevation_m = as.numeric(dplyr::slice_tail(as.data.frame(tidyhydat::hy_stn_datum_conv(i)[,4]))),
          Current_return_period = "NA"
        )
      }
  ...
  for(i in stations) {
    ...
    if (stringr::str_detect(station_info$STATION_NAME, "LAKE")==FALSE | stringr::str_detect(station_info$STATION_NAME, "RIVER")==TRUE){ #Lakes don't have flow measurements, but sometimes LAKE is mentioned when describing the river location
      ...
      tryCatch ({
        ...
        todayFlow <- flowData[[2]]$Flow[flowData[[2]]$Date == Sys.Date() & flowData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        todayFlowPerc <- flowData[[2]]$prctile[flowData[[2]]$Date == Sys.Date() & flowData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        yesterdayFlow <- flowData[[2]]$Flow[flowData[[2]]$Date == (Sys.Date() - 1) & flowData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        twodayFlow <- flowData[[2]]$Flow[flowData[[2]]$Date == (Sys.Date() - 2) & flowData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        threedayFlow <- flowData[[2]]$Flow[flowData[[2]]$Date == (Sys.Date() - 3) & flowData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        fourdayFlow <- flowData[[2]]$Flow[flowData[[2]]$Date == (Sys.Date() - 4) & flowData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        fivedayFlow <- flowData[[2]]$Flow[flowData[[2]]$Date == (Sys.Date() - 5) & flowData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        sixdayFlow <- flowData[[2]]$Flow[flowData[[2]]$Date == (Sys.Date() - 6) & flowData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        weekFlow <- flowData[[2]]$Flow[flowData[[2]]$Date == (Sys.Date() - 7) & flowData[[2]]$Year_Real == lubridate::year(Sys.Date())]
        ...
        tbl <- data.frame(
          ...
          Two_days_ago = twodayFlow,
          Three_days_ago = threedayFlow,
          `72 hr change` = todayFlow - threedayFlow,
          Four_days_ago = fourdayFlow,
          Five_days_ago = fivedayFlow,
          Six_days_ago = sixdayFlow,
          One_week_ago = weekFlow,
