...
if (report_type %in% c("Both", "Level", "both", "level")){
  ...
  for(i in stations) {
    ...
    tryCatch ({
      ...
      todayLevelPerc <- levelData$tidyData[[3]]$prctile[levelData$tidyData[[3]]$Date == Sys.Date() & levelData$tidyData[[3]]$Year_Real == lubridate::year(Sys.Date())]
      ...
      tbl <- data.frame(
        ...
        `Current level (masl)*` = round(todayLevel, 2),
        `Current percent historic` = round(todayLevelPerc, 1),
        Yesterday = yesterdayLevel,
        `24 hr change (cm)` = round((todayLevel - yesterdayLevel)*100, 1),
        `Two days ago` = twodayLevel,
        `Three days ago` = threedayLevel,
        `72 hr change (cm)` = round((todayLevel - threedayLevel)*100, 1),
        `Four days ago` = fourdayLevel,
        `Five days ago` = fivedayLevel,
        `Six days ago` = sixdayLevel,
        `One week ago` = weekLevel,
        `One week change (cm)` = round((todayLevel - weekLevel)*100, 1),
        `Datum elevation` = dplyr::slice_tail(as.data.frame(tidyhydat::hy_stn_datum_conv(i)[,4]))
      )
...
if (report_type %in% c("Both", "Level", "both", "level") & length(levelInfo) > 0){
  ...
  levelTable <- dplyr::bind_rows(levelInfo)[c(1,4,5,7,10,15,17)]
...
Current water levels are presented relative to the local datum (column 3) and relative to the Canadian Geodetic Vertical Datum 2013 (CGVD2013, column 4). The local datum is an arbitrary datum used at each hydrometric station and reported by the Water Survey of Canada at [wateroffice.ec.gc.ca](https://wateroffice.ec.gc.ca/). Local datum values can be compared at one station across years but not across stations, while CGVD2013 elevations can be compared across stations. Older (deprecated) vertical datums are published for each station on the Water Survey of Canada website.

Return periods (or repeat intervals) representing two, ten, one hundred and two hundred year returns are reported here and plotted below. A given return period refers to the expected frequency at which a specific flow or level is expected based on statistical analysis of historical records. For example, a 20-year return is expected to be met or exceeded once every 20 years or to have a 5% change of being met/exceeded every year.

```{xpected to{r Flows table, echo=FALSE, results='asis', message=FALSE}
...
stationInfo <- lapply(stations, function(x) dplyr::bind_cols(tidyhydat::hy_stations(x)[c(2,1,7,8)], dplyr::slice_tail(as.data.frame(tidyhydat::hy_stn_datum_conv(x)[,3])))) %>%
...
#Combine station Name with ID and links, called Station
...
stationInfo$Datum <- gsub("CANADIAN GEODETIC VERTICAL DATUM 2013:EPOCH2010", "CGVD2013:2010 [3]", sub)

knitr::kable(stationInfo, digits=2, caption="These are the stations contained in this report:")

```

Notes: [1] Vertical datum not specified in historical records, but implementation date indicates use of CGVD28.
