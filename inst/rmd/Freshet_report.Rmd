      ...
      tryCatch ({
        ...
        if (!(flow_returns %in% c("None", "none"))){ #if flow_returns is "none", the column is simply not created.
          ...
          if (flow_returns %in% c("both", "Both", "Table", "table") & i %in% data$flow_returns$ID){ #where possible, plug in the verified value
            ...
            tbl$Current_return_period <- 
              if (utils::tail(freq, n=1)$`Return Period` <= 2) {
                "<2 year*"
              } else if (utils::tail(freq, n=1)$`Return Period` < 5) {
                " 2 to 5 year*"
              } else if (utils::tail(freq, n=1)$`Return Period` < 10) {
                "5 to 10 year*"
              } else if(utils::tail(freq, n=1)$`Return Period` < 20) {
                "10 to 20 year*"
              } else if (utils::tail(freq, n=1)$`Return Period` < 50) {
                "20 to 50 year*"
              } else if (utils::tail(freq, n=1)$`Return Period` < 100) {
                "50 to 100 year*"
              } else if (utils::tail(freq, n=1)$`Return Period` < 200) {
                "100 to 200 year*"
              } else if (utils::tail(freq, n=1)$`Return Period` < 500) {
                "200 to 500 year*"
              } else if (utils::tail(freq, n=1)$`Return Period` < 1000) {
                "500 to 100 year*"
      ...
      if (report_type %in% c("Both", "Level", "both", "level")){
        tryCatch ({
          ...
          if (level_zoom == TRUE){ #Plot zoomed-in level data
            ...
            #   
            #   last_value <- utils::tail(levelDataList[[n]][[3]], n=1)
