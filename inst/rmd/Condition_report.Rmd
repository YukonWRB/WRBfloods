...
if (report_type %in% c("Both", "Level", "both", "level") & length(levelInfo) > 0){
  ...
  levelTable <- dplyr::bind_rows(levelInfo)[c(5, 6, 4, 8, 10, 12, 19, 20)] #select columns of level m, level masl, today percent historic, 24 hour change, 48 hour change, 72 hour change, current return period, and flags.
  ...
  colnames(levelTable) <- c("Station name (number)", "Current level (m)", "Current level (masl)", "Current percent historic", "24 hr change (cm)", "48 hr change (cm)", "72 hr change (cm)", "Current return period", "Flag")
 ...
 if (TRUE %in% stringr::str_detect(levelTable$`Current level (m)`, "M")){
    cat("M: Recent level values are missing - all data points are more than 6 hours old.")
  }
}
```
...
if (report_type %in% c("Both", "Flow", "both", "flow") & length(flowInfo) > 0){
  ...
  if (!(flow_returns %in% c("none", "None"))){
    flowTable <- dplyr::filter(flowTable, Current_flow != 0.0001)[c(1, 5, 6, 8, 10, 12, 17, 18)] #0.0001 is the indicator for lake stations, remove those
  } else {
    flowTable <- dplyr::filter(flowTable, Current_flow != 0.0001)[c(1, 5, 6, 8, 10, 12, 17)]
  }
  ...
  if (!(flow_returns %in% c("none", "None"))){
    colnames(flowTable) <- c("Station name (number)", "Current flow (m^3^/s)", "Current percent historic", "24 hr change (m^3^/s)", "48 hr change (m^3^/s)", "72 hr change (m^3^/s)", "One week change (m^3^/s)", "Current return period")
    print(knitr::kable(flowTable, digits=1, align="lccccccc"))

  } else {
    colnames(flowTable) <- c("Station name (number)", "Current flow (m^3^/s)", "Current percent historic", "24 hr change (m^3^/s)", "48 hr change (m^3^/s)", "72 hr change (m^3^/s)", "One week change (m^3^/s)")
    print(knitr::kable(flowTable, digits=1, align="lcccccc"))
  }
  ...
  if (TRUE %in% stringr::str_detect(flowTable$`Current flow (m^3^/s)`, "NA")){
   ...
   if (TRUE %in% stringr::str_detect(flowTable$`Current flow (m^3^/s)`, "M")){
    cat("M: Recent flow values are missing - all data points are more than 6 hours old.")
  }
