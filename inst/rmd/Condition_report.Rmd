  ...
  for (i in stations) {
    ...
    tryCatch ({
      ...
      if (i %in% data$level_returns$ID==TRUE) {
        ...
        tbl$Flag <- if (todayLevel >= stn$Flag_Level) {
          stn$Note
        } else {
          tbl$Flag <- "NA"
        }
    ...
    #Determine if the last data point is >6 hours old
      if (as.POSIXlt(as.numeric(levelData[[3]][nrow(levelData[[3]]),2]), origin = "1970-01-01") < (Sys.time())) {
        tbl$Last_data <- "< 1 hrs"
      }
      if (as.POSIXlt(as.numeric(levelData[[3]][nrow(levelData[[3]]),2]), origin = "1970-01-01") < (Sys.time() - 60*60*1)) {
        tbl$Last_data <- "1-2 hrs"
      }
      if (as.POSIXlt(as.numeric(levelData[[3]][nrow(levelData[[3]]),2]), origin = "1970-01-01") < (Sys.time() - 60*60*2)) {
        tbl$Last_data <- "2-3 hrs"
      }
      if (as.POSIXlt(as.numeric(levelData[[3]][nrow(levelData[[3]]),2]), origin = "1970-01-01") < (Sys.time() - 60*60*3)) {
        tbl$Last_data <- "3-4 hrs"
      }
      if (as.POSIXlt(as.numeric(levelData[[3]][nrow(levelData[[3]]),2]), origin = "1970-01-01") < (Sys.time() - 60*60*4)) {
        tbl$Last_data <- "4-5 hrs"
      }
      if (as.POSIXlt(as.numeric(levelData[[3]][nrow(levelData[[3]]),2]), origin = "1970-01-01") < (Sys.time() - 60*60*5)) {
        tbl$Last_data <- "5-6 hrs"
      }
      if (as.POSIXlt(as.numeric(levelData[[3]][nrow(levelData[[3]]),2]), origin = "1970-01-01") < (Sys.time() - 60*60*6)) {
        tbl$Last_data <- "> 6 hrs"
      }
  ...
  for(i in stations) {
    ...
    if (stringr::str_detect(station_info$STATION_NAME, "LAKE")==FALSE | stringr::str_detect(station_info$STATION_NAME, "RIVER")==TRUE){ #Lakes don't have flow measurements, but sometimes LAKE is mentioned when describing the river location
      ...
      tryCatch ({
        ...
        #Determine if the last data point is >6 hours old
        tbl$Last_data <- " "
        if (as.POSIXlt(as.numeric(flowData[[3]][nrow(flowData[[3]]),2]), origin = "1970-01-01") < (Sys.time())) {
          tbl$Last_data <- "< 1 hrs"
        }
        if (as.POSIXlt(as.numeric(flowData[[3]][nrow(flowData[[3]]),2]), origin = "1970-01-01") < (Sys.time() - 60*60*1)) {
          tbl$Last_data <- "1-2 hrs"
        }
        if (as.POSIXlt(as.numeric(flowData[[3]][nrow(flowData[[3]]),2]), origin = "1970-01-01") < (Sys.time() - 60*60*2)) {
          tbl$Last_data <- "2-3 hrs"
        }
        if (as.POSIXlt(as.numeric(flowData[[3]][nrow(flowData[[3]]),2]), origin = "1970-01-01") < (Sys.time() - 60*60*3)) {
          tbl$Last_data <- "3-4 hrs"
        }
        if (as.POSIXlt(as.numeric(flowData[[3]][nrow(flowData[[3]]),2]), origin = "1970-01-01") < (Sys.time() - 60*60*4)) {
          tbl$Last_data <- "4-5 hrs"
        }
        if (as.POSIXlt(as.numeric(flowData[[3]][nrow(flowData[[3]]),2]), origin = "1970-01-01") < (Sys.time() - 60*60*5)) {
          tbl$Last_data <- "5-6 hrs"
        }
        if (as.POSIXlt(as.numeric(flowData[[3]][nrow(flowData[[3]]),2]), origin = "1970-01-01") < (Sys.time() - 60*60*6)) {
          tbl$Last_data <- "> 6 hrs"
        }
...
)#End of trycatch for getting precip data
...
if (report_type %in% c("Both", "Level", "both", "level") & length(levelInfo) > 0){
  ...
  levelTable <- dplyr::bind_rows(levelInfo)[c(5, 6, 4, 8, 10, 12, 19, 20, 21)] #select columns of level m, level masl, today percent historic, 24 hour change, 48 hour change, 72 hour change, current return period, flags, last data.
  ...
  colnames(levelTable) <- c("Station name (number)", "Current level (m)", "Current level (masl)", "Current percent historic", "24 hr change (cm)", "48 hr change (cm)", "72 hr change (cm)", "Current return period", "Flag", "Last data age")
...
if (report_type %in% c("Both", "Flow", "both", "flow") & length(flowInfo) > 0){
  ...
  if (TRUE %in% stringr::str_detect(flowTable$`Current flow (m^3^/s)`, "NA")){
    ...
    cat("*Value obtained programatically by analysing May to September daily mean flows and fitting a log-Pearson Type 3 distribution using HEC-SSP methodology. No human checks were performed on underlying data.")
    }
}
```

```{r Rates table, echo=FALSE, results='asis', message=FALSE}
tidyhydat::hy_annual_instant_peaks("09EB001", start_year = 2010, end_year = 2022) #Sadly, full of holes and no recent
fasstr::calc_annual_peaks(station_number = "09EA004", start_year = 2010, end_year = 2022 ) #Sadly, flow only and no recent
...
colnames(precipTable) <- c("Station code", "Station name", "12 hours", "One day", "Two days", "Three days", "Four days", "One week")
  ...
  knitr::kable(precipTable, digits=3, caption=paste0("Accumulated precipitation (mm of water) prior to ", substr(validdatetime-7*60*60, 1, 16), " Yukon Time where available. Precipitation totals are generated from Environment and Climate Change Canada's HRDPA model: for more information, click [here](https://eccc-msc.github.io/open-data/msc-data/nwp_hrdpa/readme_hrdpa_en/)."), align="c")
}, error = function(e){cat("  \nThe HRDPA precipitation shapefiles could not be downloaded. Check the URL https://dd.weather.gc.ca/analysis/precip/hrdpa_watershed/shapefile/06/ first to see if it can be reached, if that checks out ask Ghislain to look further into the problem.")})
```
