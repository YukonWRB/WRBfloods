    ...
    if (stringr::str_detect(station_info$STATION_NAME, "LAKE")==FALSE | stringr::str_detect(station_info$STATION_NAME, "RIVER")==TRUE){ #Lakes don't have flow measurements, but sometimes LAKE is mentioned when describing the river location
      ...
      tryCatch ({
        ...
        if (!(flow_returns %in% c("None", "none"))){ #if flow_returns is "none", the column is simply not created.
          ...
          if (flow_returns %in% c("both", "Both", "Calc", "calc") & i %in% data$flow_returns$ID){ #where possible, plug in the verified value
          ...
          } else if (flow_returns %in% c("Calc", "calc") & !(i %in% data$flow_returns$ID)){ #if humnan-verified values only requested but not available, give NA
...
if (report_type %in% c("Both", "Level", "both", "level") & length(levelInfo) > 0){
  ...
  knitr::kable(levelTable, digits=2, align = "lcccccccc")
}
```
...
if (report_type %in% c("Both", "Flow", "both", "flow") & length(flowInfo) > 0){
  ...
  #TODO: check to make sure this line works!
  if (TRUE %in% stringr::str_detect(flowTable$`Current return period`, "*")){
    cat("*Value obtained programatically by analysing May to September daily mean flows. No human checks were performed on underlying data.")
  }
      ...
      if (report_type %in% c("Both", "Level", "both", "level")){
        tryCatch ({
          ...
          if (level_zoom == TRUE){ #Plot zoomed-in level data
            ...
            #   
            #   #TODO: Confirm no need for adjustment to CLEVER
