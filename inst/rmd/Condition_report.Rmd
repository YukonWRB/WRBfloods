...
if (report_type %in% c("Both", "Level", "both", "level")){
  ...
  for(i in stations) {
    ...
    tryCatch ({
      ...
      #next few lines calculate where we're at for return periods and if there's a flag raised
...
if (report_type %in% c("Both", "Flow", "both", "flow") & length(flowInfo) > 0){
  ...
  flowTable <- dplyr::filter(flowTable, Current_flow != 0.0001)[c(1, 5, 6, 8, 10, 12, 17)] #0.0001 is the indicator for lake stations, remove those
