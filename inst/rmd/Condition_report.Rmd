      ...
      if (report_type %in% c("Both", "Level", "both", "level")){
        tryCatch ({
          ...
          if (level_zoom == TRUE){ #Plot zoomed-in level data
            ...
            # #Lines below will be useful if and when lake levels are part of the MESH or CLEVER outputs. In the meantime it is not functional. 
            # if (i %in% names(MESH) & i %in% names(CLEVER)){
            #   #Add in the MESH and CLEVER data, then nudge it to match WSC at forecast time.
            #   levelDataList[[n]][[3]] <- dplyr::full_join(levelDataList[[n]][[3]], MESH[[i]], by = c("Date" = "datetime"))
            #   levelDataList[[n]][[3]] <- dplyr::full_join(levelDataList[[n]][[3]], CLEVER[[i]], by = c("Date" = "datetime"))
            #   
            # NUDGE THE MESH DATA BEFORE NEXT STEP IF NECESSARY
