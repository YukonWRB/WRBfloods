...
stationInfo <- lapply(stations, function(x) dplyr::bind_cols(tidyhydat::hy_stations(x)[c(2,1,7,8)], dplyr::slice_tail(as.data.frame(tidyhydat::hy_stn_datum_conv(x)[,3])))) %>%
  ...
  plyr::rename(c("STATION_NAME"="Station Name", "STATION_NUMBER"="ID", "LATITUDE"="Latitude", "LONGITUDE"="Longitude", "DATUM_TO"="Datum"))
...
stationInfo$Datum <- gsub("CANADIAN GEODETIC VERTICAL DATUM 2013:EPOCH2010", "CGVD2013:2010", sub)
