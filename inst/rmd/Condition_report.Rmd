...
if (report_type %in% c("Both", "Flow", "both", "flow") & length(flowInfo) > 0){
  ...
  if (nrow(flowTable)  > 0){
    ...
    if (TRUE %in% stringr::str_detect(flowTable$`Current return period`, "\\*")){
      cat("*Value obtained programatically by analysing May to September daily mean flows and fitting a log-Pearson Type 3 distribution using HEC-SSP methodology. For informational purposes only:o human checks were performed on underlying data.")
    }
  }
