...
#' @param forecast Do you want MESH or CLEVER forecasts, or both? Choose from "MESH", "CLEVER", "both", or "none".
...
flowPlot <- function(station, years, title=TRUE, zoom=FALSE, zoom_days=30, filter=FALSE, forecast = "none", returns = "both", save_path="choose") {
  ...
  if (forecast %in% "MESH"){
    ...
    #Extract information from the files and stick it into a single data.frame:
      ...
      if (station %in% names(QOMEAS)){
        ...
        MESH$DateOnly <- as.Date(substr(stn$datetime, 1, 10))
        ...
        plot <- utils_zoom_flow_plot(station_number = station,
                                     ...
                                     returns = returns,
                                     complete_df = flowData[[1]])
        print ("No MESH prediction could be found for this station. A plot without forecast flows has been produced instead.")
      }
  }
  ...
  if (forecast %in% "CLEVER"){ #get the CLEVER data and add it to flowData[[1]]
    ...
    exists <- RCurl::url.exists(paste0("http://bcrfc.env.gov.bc.ca/freshet/clever/", station, ".CSV"))
    ...
    if (exists == TRUE){
      ...
      print ("No CLEVER prediction could be found for this station. A plot without forecast flows has been produced instead.")
