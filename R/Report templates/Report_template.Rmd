...
```{r Fixed camera images, echo=FALSE, fig.height=4, fig.width=7, message=FALSE, warning=FALSE, fig.height=4, fig.width=7, results='asis'}
...
if (nrow(images)>=1){
  ...
  for (i in unique(images$Station)){
    cat("  \n## ", stringr::str_to_title(hy_stations(i)$STATION_NAME))
    
    last_image <- dplyr::filter(images, Station==i)[1,1]
    
    temp <- paste0(tempfile(),".jpg")
    R.utils::downloadFile(paste0("https://collaboration.cmc.ec.gc.ca/cmc/hydrometric_additionalData/FieldData/YT/", last_image), temp, username=Sys.getenv("ECCCUSER"), password = Sys.getenv("ECCCPASS"))
