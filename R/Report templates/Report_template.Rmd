...
if (report_type %in% c("Both", "Level", "both", "level")){
  ...
  for(i in stations) {
    ...
    tbl <- data.frame(
      `Station name` = if(is.na(station_info$STATION_NUMBER[1])==FALSE){paste0("Station ", stringr::str_to_title(tidyhydat::hy_stations(i)$STATION_NAME), " has no current level")} else{paste0("Station ", stringr::str_to_title(tidyhydat::hy_stations(i)$STATION_NAME), " does not exist")},
      ID = i,
      `Start year` = NA,
...
if (report_type %in% c("Both", "Flow", "both", "flow")){
  ...
  for(i in stations) {
    tbl <- data.frame(
      `Station name` = if(is.na(station_info$STATION_NUMBER[1])==FALSE){paste0("Station ", stringr::str_to_title(tidyhydat::hy_stations(i)$STATION_NAME), " has no current flow")} else{paste0("Station ", stringr::str_to_title(tidyhydat::hy_stations(i)$STATION_NAME), " does not exist")},
      ID = i,
      `Start year` = NA,
      `Today percent historic` = NA,
      Today = NA,
...
knitr::kable(levelTable, caption="Summary of water levels and level changes in meters:", digits=3)
```
...
knitr::kable(flowTable, caption="Summary of water flows and flow changes in cubic meters per second:", digits=3)
```
...
```{r Generate plots, echo=FALSE, fig.height=4, fig.width=7, message=FALSE, warning=FALSE, results='asis'}
