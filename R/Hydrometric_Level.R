...
level_data <- function(station_number,
                       ...
                       csv_path = NA) # or provide path to folder as text string
                       {
                         ...
                         leap_list <-
                           (seq(1800, 2100, by = 4))  # Create list of all leap years
                         
                         level_historic <-
                           (tidyhydat::hy_daily_levels(station_number = station_number)
                            [, -c(3, 5)])
                         ...
                         if (extract_realtime == T) {
                           if (max(select_years) >= lubridate::year(Sys.Date() - 730)) {
                             level_real_time <-
                               tidyhydat.ws::realtime_ws(
                                 station_number = as.character(station_number),
                                 parameters = 46,
                                 start_date = ifelse(
                                   max(lubridate::year(level_historic$Date)) == lubridate::year(Sys.Date() - 730),
                                   paste(paste(lubridate::year(
                                     Sys.Date() - 365
                                   )), "01", "01", sep = "-"),
                                   paste(paste(lubridate::year(
                                     Sys.Date() - 730
                                 ...
                                 end_date = ifelse(
                                   lubridate::year(Sys.Date()) > max(select_years),
                         ...
                         level_df <-
                           fasstr::fill_missing_dates(data = level_df,
                                                      dates = "Date",
                                                      value = "Level")
                         ...
                         level_df <-
                           level_df[!(format(level_df$Date, "%m") == "02" &
                                        format(level_df$Date, "%d") == "29"), , drop = FALSE]
                         ...
                         level_df <- level_df %>%
                           dplyr::mutate(dayofyear = ifelse(
                             lubridate::year(Date) %in% leap_list,
                             ifelse(
                       ...
                       level_plot <- function(station_number,
                                              complete_year,
                                              plot_years_df,
                                              dummy_year_df,
                                              png_path = NA,
                       ...
                       
                       {
                         ...
                         legend_length <-
                           length(legend_length$Year_Real) - 1 # The '-1' accounts for the NA column
                         ...
                         plot <-
                           ggplot2::ggplot(all_data, ggplot2::aes(x = Date, y = Value)) +
                           ggplot2::labs(
                             title = paste0(station$STATION_NAME, " (", station$STATION_NUMBER, ")"),
                             x = "Month",
                             y = expression(paste("Level (masl)"))
                           ) +
                           ggplot2::scale_x_date(date_breaks = "1 months",
                                                 labels = scales::date_format("%b")) +
                           tidyquant::coord_x_date(xlim = c(
                             paste(complete_year, "-01-01", sep = ""),
                         ...
                         if (percentile_plot == TRUE &&
                           ...
                           plot <- plot +
                             ...
                             ggplot2::geom_line(
                               data = all_data,
                               ggplot2::aes(colour = prctile, group = Year_Real),
                               size = line_size,
                               na.rm = T
                             ) +
                             ggplot2::scale_colour_gradientn(
                               name = paste0(select_years,
                                             "\nPercentiles"),
                               limits = c(0, 100),
                               colours = rainbow(3)
                             ) +
                           ...
                           plot <- plot +
                             ggplot2::geom_point(
                               ggplot2::aes(colour = factor(Year_Real)),
                               shape = 19,
                               size = point_size,
                               na.rm = T
                             ) +
                             ggplot2::geom_line(ggplot2::aes(colour = factor(Year_Real)),
                                                size = line_size,
                                                na.rm = T) +
                         ...
                         if (percentile_plot == T && length(select_years) > 1) {
                           print(
                             "ERROR: percentile_plot is only able to present one year of data. Reduce select_years to one year"
                           )
                         } else {
                           if (!is.na(png_path)) {
                             ggplot2::ggsave(
