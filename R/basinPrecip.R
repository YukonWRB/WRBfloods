                        ...
                        )
{
  ...
  if (hrdpa == TRUE){
    ...
    if (min(sequence_hrdpa) < last_available_01){ #TRUE means that there is at least one HRDPA available online or that there is one locally, so fetch it. Otherwise, no HRDPA is available yet and only HRDPS forecast will be used.
      ...
      if (!is.null(hrdpa_loc)){ #Get the list of available files locally, if hrdpa_loc != NULL
        ...
        if(length(missing_time) > 0){
          if(is.na(have_time$extent[1])){
            ...
            for (i in 1:length(missing_time)){
              WRBtools::getHRDPA(start = missing_time[i], end = missing_time[i], clip = smallest, save_path = hrdpa_loc)
            }
          } else {
            for (i in 1:length(missing_time)){
              WRBtools::getHRDPA(start = missing_time[i], end = missing_time[i], clip = unique(have_time$extent)[1], save_path = hrdpa_loc)
            }
          }
        }
        ...
        if (nrow(missing_extent) > 0){
          if (is.na(have_extent$extent[1])){
            ...
            for (i in 1:length(missing_extent)){
              WRBtools::getHRDPA(start = missing_extent[i,2], end = missing_extent[i,2], clip = smallest, save_path = hrdpa_loc)
            } 
          } else {
            for (i in 1:length(missing_extent)){
              WRBtools::getHRDPA(start = missing_extent[i,2], end = missing_extent[i,2], clip = unique(have_extent$extent)[1], save_path = hrdpa_loc)
            }
          }
        }
        ...
        if(length(missing_time) > 0){
          for (i in 1:length(missing_time)){
            WRBtools::getHRDPA(start = missing_time[i], end = missing_time[i], clip = NULL, save_path = hrdpa_loc)
          }
        }
  ...
  if (type == "longlat"){
    list <- list(mean_precip = mean_precip, total_time_range = actual_times, reanalysis_time_range = actual_times_hrdpa, forecast_time_range = actual_times_hrdps, point = requested_point, plot = if(map) plot else NULL)
    ...
    list <- list(mean_precip = mean_precip, min = min, max = max, total_time_range = actual_times, reanalysis_time_range = actual_times_hrdpa, forecast_time_range = actual_times_hrdps, watershed = watershed$StationNum, plot = if(map) plot else NULL)
